...
func (s *authService) Register(ctx context.Context, req model.RegisterRequest) (model.TokenResponse, error) {
	...
	if err != nil {
		...
		return model.TokenResponse{}, errors.New("internal registration error")
	}
	...
	if err := s.repo.Register(ctx, userData); err != nil {
		...
		return model.TokenResponse{}, fmt.Errorf("repository error: %w", err)
	}
	accessToken, err := jwtutil.GenerateToken(userData.UserID, userData.Username, 15*time.Minute)
	if err != nil {
		s.logger.Error("failed to generate access token", "error", err)
		return model.TokenResponse{}, errors.New("internal server error")
	}
	refreshToken, err := jwtutil.GenerateToken(userData.UserID, userData.Username, 30*24*time.Hour)
...
func (s *authService) Refresh(ctx context.Context, refreshToken string) (model.TokenResponse, error) {
	...
	newAccess, _ := jwtutil.GenerateToken(claims.UserID, claims.Username, 15*time.Minute)
	newRefresh, _ := jwtutil.GenerateToken(claims.UserID, claims.Username, 30*24*time.Hour)
	return model.TokenResponse{
